...
export default function MeterScheduling() {
  ...
  useEffect(() => {
    const fetchMeterRequests = async () => {
      try {
        ...
        console.log("API response for meter requests:", res.data);
  ...
  const handleAccept = async (id) => {
    try {
      await axios.put(`http://localhost:5000/api/schedule/meter/${id}/accept`, null, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const handleReject = async (id) => {
    try {
      await axios.put(`http://localhost:5000/api/schedule/meter/${id}/reject`, null, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
