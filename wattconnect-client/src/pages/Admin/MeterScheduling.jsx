...
export default function MeterScheduling() {
  ...
  useEffect(() => {
    // Fetch all pending meter installation requests from the backend
    axios.get('/api/schedule/meter')
      .then(res => setRequests(res.data))
      .catch(err => console.error(err));
  }, []);

  const handleAccept = (id) => {
    axios.put(`/api/schedule/meter/${id}/accept`)
      .then(() => {
  ...
  const handleReject = (id) => {
    axios.put(`/api/schedule/meter/${id}/reject`)
      .then(() => {
        setRequests(prev => prev.filter(r => r._id !== id));
        setSelectedRequest(null);
      });
