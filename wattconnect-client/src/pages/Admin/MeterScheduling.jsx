...
export default function MeterScheduling() {
  ...
  useEffect(() => {
    const fetchMeterRequests = async () => {
      try {
        ...
        console.error("Error fetching meter requests:", err);
      }
    };
  ...
  const handleAccept = async (id) => {
    try {
      await axios.put(`/api/schedule/meter/${id}/accept`, null, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const handleReject = async (id) => {
    try {
      await axios.put(`/api/schedule/meter/${id}/reject`, null, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
    ...
    <div className="flex min-h-screen bg-[#f4f6fa]">
      ...
      <main className="flex-1 p-6 relative">
        <h1 className="text-2xl font-bold mb-6">
          Check the pending meter installation schedule requests here!
        </h1>
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 ${selectedRequest ? 'blur-sm pointer-events-none' : ''}`}>
          {requests.map((request) => (
            <div key={request._id} className="bg-white shadow-md rounded-lg p-4">
              <p className="font-semibold text-gray-800 mb-1">{request.applicantName || request.name}</p>
              <p className="text-sm text-gray-600">Consumer #: {request.consumerNumber}</p>
              <p className="text-sm text-gray-600 mb-4">Purpose: {request.usageType || request.purpose}</p>
              <button
        ...
        {selectedRequest && (
          <div className="fixed inset-0 z-50 bg-white p-6 md:p-10 overflow-y-auto shadow-lg rounded-lg mx-auto max-w-2xl border border-gray-300">
            <h2 className="text-2xl font-bold mb-4">Meter Installation Details</h2>
            <div className="space-y-2 text-gray-700">
              ...
              <p><strong>Name:</strong> {selectedRequest.applicantName || selectedRequest.name}</p>
              <p><strong>Address:</strong> {selectedRequest.address}</p>
              <p><strong>Purpose:</strong> {selectedRequest.usageType || selectedRequest.purpose}</p>
