...
export default function CustomerDashboard() {
  ...
  const totalStepsByShaan = interpolationChartData.datasets[0].data.reduce((sum, val) => sum + val, 0);
  const totalStepsByShweta = interpolationChartData.datasets[1].data.reduce((sum, val) => sum + val, 0);
        ...
        <div className="bg-white p-6 rounded-2xl shadow-md w-full flex flex-col md:flex-row gap-6 max-w-5xl">
          ...
          <div className="flex flex-col gap-6 md:w-1/4">
            <div className="rounded-xl p-4 shadow-md">
              ...
              <h3 className="text-2xl font-semibold text-gray-700 mt-1">{totalStepsByShaan.toLocaleString()} steps</h3>
            </div>

            <div className="rounded-xl p-4 shadow-md">
              <p className="text-sm text-gray-500">Total Steps by Shweta</p>
              <h3 className="text-2xl font-semibold text-gray-700 mt-1">{totalStepsByShweta.toLocaleString()} steps</h3>
            </div>
          </div>

          {/* Right Column: Line Chart */}
          <div className="w-full md:w-2/3">
            <h3 className="text-xl font-semibold text-gray-700 mb-4">Monthly Steps Trend</h3>
            <LineChart />
          </div>
        </div>
      </main>
